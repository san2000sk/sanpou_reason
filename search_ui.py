...
st.markdown("""
    ...
    /* Streamlitのデフォルトヘッダーと「Manage app」ボタンを非表示にする試み */
    [data-testid="stHeader"], footer {
        display: none !important;
    }
    /* 右下のManage appメニューを隠す */
...
def format_date(date_str):
    ...
    for p in parts:
        if len(p) == 1:
            formatted_parts.append(to_full_width(p))
        else:
...
with left:
    ...
    with col1_:
        if st.button("検索", use_container_width=True, type="primary"):
            ...
            st.session_state["selected_ids"] = [] # 検索時にリセット
    with col2_:
...
with right:
    if st.session_state.get('search', False):
        ...
        if not df.empty:
            ...
            # --- 書き出し機能セクション ---
            st.markdown(f"<div style='display: flex; justify-content: space-between; align-items: center;'><div><b>該当件数：{result_count} 件</b></div></div>", unsafe_allow_html=True)
            ...
            if selected_labels:
                ...
                with col_dl2:
                    ...
                    for label in selected_labels:
                        ...
                        num_raw = parts[1]
                        num_int = int(num_raw)
                        num_full = to_full_width(num_int)
                        
                        date_formatted = format_date(row['submitted_date'])
                        
                        output_text += f"◯{row['title']}（第{round_num}回国会参法第{num_full}号・{date_formatted}提出）\n"
            ...
            html = """
            <style>
            .scroll-box { max-height: 500px; overflow-y: auto; border: 1px solid #ccc; background-color: #fcfcfc; }
            ...
            with p_col1:
                if st.button("◀ 前へ", use_container_width=True, disabled=(st.session_state["page"] <= 1), key="prev_btn"):
                    ...
                    st.rerun()
            with p_col2:
                st.markdown(f"<div style='text-align: center; padding-top: 5px; font-weight: bold;'>ページ {st.session_state['page']} / {st.session_state['total_pages']}</div>", unsafe_allow_html=True)
            with p_col3:
